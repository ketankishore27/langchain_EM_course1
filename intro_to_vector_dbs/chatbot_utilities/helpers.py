...
def get_rag_agent(model, index_name, embeddings, chat_history = []):
    ...
    history_aware = create_history_aware_retriever(llm = llm, 
                                                   retriever=vector_store.as_retriever(), 
                                                   prompt = rephrase_prompt)
    
    retrieval_qa_agent = create_retrieval_chain(retriever = history_aware, 
...
if __name__ == "__main__":
    load_dotenv()
    retrieval_qa_agent = get_rag_agent(model = "gpt-4o-mini", index_name = "langchain-chabot", embeddings = "text-embedding-3-small")
    answer = retrieval_qa_agent.invoke({"input": "How to use configure json parser in langchain"})['answer']
    print("\n\n*********")
    print(answer)
    print("\n\n*********")
